...
# titles = ['dX', 'dY', 'dZ']
# for i in range(3):
#     axes[i].plot(trues[:, i], label='True')
...
# titles = ['dX', 'dY', 'dZ']
# colors = plt.cm.viridis(np.linspace(0, 1, len(trues)))

# for i in range(3):
#     for j in range(len(trues)):
#         axes[i].scatter(trues[j, i], preds[j, i], color=colors[j], s=25)
#     axes[i].set_xlabel("Ground Truth")
#     axes[i].set_ylabel("Prediction")
#     axes[i].set_title(titles[i])
#     axes[i].grid(True)

# plt.suptitle("Validation Prediction vs Ground Truth (Scatter)")
# plt.tight_layout()
# plt.show()
